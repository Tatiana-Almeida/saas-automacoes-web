...
class AuthFlowTests(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="admin@example.com", password=self.password, role="admin"
        )
    ...
    def test_password_reset_confirm(self):
        ...
        res = self.client.post(
            url, {"uid": uid, "token": token, "new_password": new_pwd}, format="json"
        )
        self.assertEqual(res.status_code, 200)
        # login with new password
        obtain_url = reverse("token_obtain_pair")
        res_login = self.client.post(
            obtain_url, {"email": self.user.email, "password": new_pwd}, format="json"
        )
    ...
    def test_password_reset_confirm_invalid_token(self):
        ...
        res = self.client.post(
            url,
            {"uid": uid, "token": bad_token, "new_password": token_urlsafe(12) + "A1!"},
            format="json",
        )
        self.assertEqual(res.status_code, 400)
    ...
    def test_password_reset_confirm_invalid_uid(self):
        ...
        res = self.client.post(
            url,
            {
                "uid": "bad-uid",
                "token": token,
                "new_password": token_urlsafe(12) + "A1!",
            },
            format="json",
        )
        self.assertEqual(res.status_code, 400)
