...
from starter.orders.models import Cart, Order, OrderStatus, PaymentTransaction, PaymentStatus
import secrets
...
class OrdersFlowTests(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(email='user@example.com', password=test_password, role='cliente')
        self.product = Product.objects.create(name='Auto001', description='Desc', price='99.90', is_active=True)
        # auth
        token_url = reverse('token_obtain_pair')
        res = self.client.post(token_url, {'email': self.user.email, 'password': test_password}, format='json')
    ...
    def test_cart_add_update_remove_and_checkout(self):
        ...
        res_add = self.client.post(add_url, {'product_id': self.product.id, 'quantity': 2}, format='json')
        ...
        self.client.post(add_url, {'product_id': self.product.id, 'quantity': 1}, format='json')
    ...
    def test_webhook_endpoint_public(self):
        ...
        res = self.client.post(webhook_url, {
            'provider': 'stripe',
            'order_id': 999,
            'status': 'succeeded'
        }, format='json')
        self.assertIn(res.status_code, (200, 400))  # accepts request (may fail validation)
    ...
    def test_payment_failure_scenario(self):
        ...
        free_product = Product.objects.create(name='Free', description='Free item', price='0.00', is_active=True)
        add_url = reverse('cart')
        self.client.post(add_url, {'product_id': free_product.id, 'quantity': 1}, format='json')
        checkout_url = reverse('orders-checkout')
        res = self.client.post(checkout_url, {'provider': 'local'}, format='json')
        self.assertEqual(res.status_code, 201)
