...
def run_automation_task(automation_id: int, log_id: int = None):
    ...
    try:
        if log_id:
            ...
            log = AutomationLog.objects.create(automation=automation, status=AutomationRunStatus.STARTED)
        ...
        msg = str(e)
        if log is None:
            log = AutomationLog.objects.create(automation=automation, status=AutomationRunStatus.FAILED)
...
def schedule_automations_task():
    ...
    for a in automations:
        ...
        if cron_expr and croniter is not None:
            ...
            try:
                ...
                if isinstance(next_time, datetime) and timezone.is_naive(next_time):
                    next_time = timezone.make_aware(next_time, timezone.get_current_timezone())
        ...
        if not due:
            continue
        log = AutomationLog.objects.create(automation=a, status=AutomationRunStatus.STARTED, started_at=now)
