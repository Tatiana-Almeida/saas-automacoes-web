...
class AutomationFlowTests(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(email='user@example.com', password=token_urlsafe(12)+'A1!', role='cliente')
        token_url = reverse('token_obtain_pair')
        res = self.client.post(token_url, {'email': self.user.email, 'password': self.user._password}, format='json')
        ...
        self.user.save()
        res = self.client.post(token_url, {'email': self.user.email, 'password': pwd}, format='json')
    ...
    def test_scheduler_triggers_due_automation(self):
        ...
        create_url = reverse('automation-list')
        payload = {'name': 'Scheduled WhatsApp', 'type': 'whatsapp', 'configuration': {'interval_minutes': 0}}
