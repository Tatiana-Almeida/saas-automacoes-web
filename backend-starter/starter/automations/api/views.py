...
from .serializers import AutomationSerializer, AutomationLogSerializer
...
class AutomationViewSet(TenantScopedViewSet):
    ...
    def trigger(self, request, pk=None):
        ...
        if not automation.is_active:
            return Response({"detail": "Automação está pausada."}, status=status.HTTP_400_BAD_REQUEST)
        # Start log as STARTED
        log = AutomationLog.objects.create(automation=automation, status=AutomationRunStatus.STARTED, started_at=timezone.now())
    ...
    @action(detail=False, methods=['get'], url_path='reports/summary', url_name='report-summary')
    def report_summary(self, request):
        ...
        recent_logs = AutomationLog.objects.filter(automation__in=qs, created_at__gte=last_24h)
