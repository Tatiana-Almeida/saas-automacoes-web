...
def send_notification_task(notification_id: int):
    ...
    try:
        if notif.channel == 'email':
            ...
            output = svc.send_sms(cfg.get('api_url',''), cfg.get('token',''), notif.to, notif.body)
        elif notif.channel == 'whatsapp':
            cfg = notif.payload or {}
            output = svc.send_whatsapp(cfg.get('api_url',''), cfg.get('token',''), notif.to, notif.body)
        # success
        ...
        NotificationLog.objects.create(notification=notif, status=NotificationStatus.SENT, attempt=attempt, response_payload=output, metrics={k: output.get(k) for k in ("elapsed_ms","attempts","last_attempt_ms","status_code") if k in output})
    except Exception as e:  # noqa
        notif.status = NotificationStatus.FAILED
        notif.attempts = attempt
        notif.last_error = str(e)
        notif.save(update_fields=["status","attempts","last_error"])
        NotificationLog.objects.create(notification=notif, status=NotificationStatus.FAILED, attempt=attempt, message=str(e))


@shared_task
def send_event_notification_task(event: str, user_id: int, title: str, body: str, channel: str = 'email', payload: dict | None = None):
    user = User.objects.get(id=user_id)
    to = user.email if channel == 'email' else payload.get('to') if payload else ''
    notif = Notification.objects.create(user=user, channel=channel, to=to or '', title=title, body=body, payload=payload or {})
