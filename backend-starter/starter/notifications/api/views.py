...
class EventNotificationsView(APIView):
    ...
    def post(self, request):
        ...
        if not event or not user_id:
            return Response(
                {"detail": "event and user_id required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        task_id = send_event_notification_task.delay(
            event, user_id, title, body, channel, payload
        )
        return Response(
            {"task_id": task_id.id, "detail": "Queued"}, status=status.HTTP_202_ACCEPTED
        )
