...
from starter.notifications.models import Notification, NotificationStatus
...
class NotificationTests(TestCase):
    def setUp(self):
        ...
        self.admin = User.objects.create_user(email='admin@example.com', password=self.admin_pwd, role='admin', is_staff=True)
        # client
        self.user_pwd = token_urlsafe(12)+'A1!'
        self.user = User.objects.create_user(email='user@example.com', password=self.user_pwd, role='cliente')
        # auth admin
        token_url = reverse('token_obtain_pair')
        res = self.client.post(token_url, {'email': self.admin.email, 'password': self.admin_pwd}, format='json')
    ...
    def test_events_enqueue_endpoint(self):
        ...
        payload = {'event': 'payment_succeeded', 'user_id': self.user.id, 'title': 'Pagamento aprovado', 'body': 'Obrigado!'}
        res = self.client.post(url, payload, format='json')
    ...
    def test_user_history_mine_filter(self):
        # auth as user
        token_url = reverse('token_obtain_pair')
        res = self.client.post(token_url, {'email': self.user.email, 'password': self.user_pwd}, format='json')
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {res.data['access']}")
        # Create a notification for user
        n = Notification.objects.create(user=self.user, channel='email', to=self.user.email, title='Teste', body='Body')
