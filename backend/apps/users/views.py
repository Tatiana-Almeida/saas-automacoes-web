    ...
    def get(self, request):
        ...
        payload = {
            ...
            'username': (
                ...
                or (lambda e: e.split('@', 1)[0] if e and e.endswith('@noemail.invalid') else user.get_username())(getattr(user, 'email', None))
            ),
            'id': user.id,
...
class AdminPingView(APIView):
    ...
    @swagger_auto_schema(
        ...
        responses={200: openapi.Schema(type=openapi.TYPE_OBJECT, properties={'ok': openapi.Schema(type=openapi.TYPE_BOOLEAN), 'admin': openapi.Schema(type=openapi.TYPE_BOOLEAN)})},
        tags=['auth']
    )
    def get(self, request):
...
class RegisterView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            201: OpenApiExample('created', value={"id": 1, "username": "user"})
        },
        tags=['auth']
    )
    @swagger_auto_schema(
    ...
    def post(self, request):
        ...
        try:
            ...
            pass
        return Response({"id": user.id, "username": user.get_username()}, status=status.HTTP_201_CREATED)
...
class LogoutView(APIView):
    ...
    @swagger_auto_schema(
        ...
        responses={
            200: openapi.Schema(type=openapi.TYPE_OBJECT, properties={'detail': openapi.Schema(type=openapi.TYPE_STRING)}),
    ...
    def post(self, request):
        ...
        try:
            ...
            token_str = serializer.validated_data.get('refresh') if hasattr(serializer, 'validated_data') else None
        ...
        try:
            ...
            security_logger.info(
                'user_logout',
                extra={
                    'username': getattr(getattr(request, 'user', None), 'get_username', lambda: None)(),
                    'tenant_schema': getattr(tenant, 'schema_name', None),
                    'ip': getattr(request, 'META', {}).get('REMOTE_ADDR'),
                    'path': request.path,
                }
        ...
        try:
            ...
            try:
                ...
                if uname and uname_field and uname_field not in data:
                    ...
                    data[uname_field] = uname if '@' in uname else f"{uname}@noemail.invalid"
