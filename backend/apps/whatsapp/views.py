...
class WhatsappSendMessageView(APIView):
    ...
    @swagger_auto_schema(
        ...
        responses={
            201: openapi.Response(
                ...
                examples={"application/json": {"queued": True, "to": "+351900000000", "task_id": "celery-task-id"}}
            )
        },
        tags=['whatsapp']
    )
    def post(self, request):
        ...
        return Response({"queued": True, "to": data["to"], "task_id": task.id}, status=status.HTTP_201_CREATED)
