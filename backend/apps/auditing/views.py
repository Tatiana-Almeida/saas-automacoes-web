    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter(
                "created_after",
                openapi.IN_QUERY,
                description="ISO datetime filter (gte)",
                type=openapi.TYPE_STRING,
            ),
            openapi.Parameter(
                "created_before",
                openapi.IN_QUERY,
                description="ISO datetime filter (lte)",
                type=openapi.TYPE_STRING,
            ),
            openapi.Parameter(
                "ordering",
...
class AuditRetentionPolicyListCreateView(APIView):
    ...
    @extend_schema(
        request=AuditRetentionPolicySerializer,
        responses=AuditRetentionPolicySerializer,
        tags=["auditing"],
    )
    @swagger_auto_schema(
...
class AuditRetentionPolicyDetailView(APIView):
    ...
    def put(self, request, policy_id):
        try:
            ...
            return Response(
                {"detail": "Policy not found"}, status=status.HTTP_404_NOT_FOUND
            )
        ser = AuditRetentionPolicySerializer(
