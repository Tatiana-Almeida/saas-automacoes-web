...
@shared_task
def export_audit_logs_to_elasticsearch(batch_size: int = 1000, minutes_back_if_empty: int = 5, max_attempts: int = 3, base_backoff_seconds: float = 1.0):
    ...
    qs = AuditLog.objects.filter(created_at__gt=since).order_by('created_at')[:batch_size]
    logs = list(qs)
    if not logs:
    ...
    if status_code != 200:
        return {'status': 'error', 'code': status_code, 'attempts': attempt, 'body': body[:500]}
...
def send_audit_alert(log_id: int):
    ...
    tenant_schema = getattr(log, 'tenant_schema', None)
    action = getattr(log, 'action', None)
    bypass_actions = set(getattr(settings, 'ALERT_WEBHOOK_QUIET_BYPASS_ACTIONS', []) or [])
    ...
    if quiet_minutes > 0 and action not in bypass_actions:
        ...
        if cache.get(action_key):
            ...
            if last_dt_action >= (getattr(log, 'created_at', timezone.now()) - timedelta(seconds=0)):
                if (timezone.now() - last_dt_action) < timedelta(seconds=window_seconds):
