...
class AuditLogAdmin(admin.ModelAdmin):
    list_display = ("created_at", "user", "method", "source", "action", "status_code", "tenant_schema", "path", "ip_address", "has_payload")
    list_filter = ("method", "source", "action", "tenant_schema")
    search_fields = ("path", "user__username")
    date_hierarchy = "created_at"
    ordering = ("-created_at",)
    readonly_fields = ("user", "path", "method", "source", "action", "status_code", "tenant_schema", "tenant_id", "ip_address", "created_at")
    ...
    def export_selected_as_csv(self, request, queryset):
        ...
        writer.writerow(['created_at', 'username', 'method', 'source', 'action', 'status_code', 'tenant_schema', 'path', 'ip_address', 'payload'])
        for log in queryset.select_related('user'):
            try:
                import json
                payload_str = json.dumps(getattr(log, 'payload', None), ensure_ascii=False) if getattr(log, 'payload', None) is not None else ''
            except Exception:
                payload_str = ''
            writer.writerow([
    ...
    def pretty_payload(self, obj):
        ...
        try:
            ...
            pretty = str(data)
        return format_html('<pre style="white-space: pre-wrap; max-width:100%; overflow:auto;">{}</pre>', pretty)
