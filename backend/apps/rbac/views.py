...
class UserRoleAssignView(APIView):
    ...
    @extend_schema(request=AssignRoleSerializer, responses={200: UserRoleSerializer}, tags=['rbac'])
    @swagger_auto_schema(
        operation_description="Atribui uma role a um usuário no tenant atual",
        manual_parameters=[
            openapi.Parameter('user_id', openapi.IN_PATH, description='ID do usuário alvo', type=openapi.TYPE_INTEGER),
        ],
    ...
    def post(self, request, user_id):
        ...
        if tenant is None:
            return Response({'detail': 'Tenant não disponível no contexto'}, status=status.HTTP_400_BAD_REQUEST)
...
class UserPermissionAssignView(APIView):
    ...
    @extend_schema(request=AssignPermissionSerializer, responses={200: UserPermissionSerializer}, tags=['rbac'])
    @swagger_auto_schema(
    ...
    def post(self, request, user_id):
        ...
        up, _ = UserPermission.objects.get_or_create(user=target, permission=perm, tenant=tenant)
        try:
            AuditLog.objects.create(
                user=getattr(request, 'user', None),
                path=request.path,
...
class UserPermissionRevokeView(APIView):
    ...
    @extend_schema(request=AssignPermissionSerializer, responses={204: None}, tags=['rbac'])
    def post(self, request, user_id):
        try:
            target = User.objects.get(id=user_id)
        except User.DoesNotExist:
        ...
        if tenant is None:
            return Response({'detail': 'Tenant não disponível no contexto'}, status=status.HTTP_400_BAD_REQUEST)
        from .models import UserPermission
        UserPermission.objects.filter(user=target, permission=perm, tenant=tenant).delete()
        try:
            AuditLog.objects.create(
                user=getattr(request, 'user', None),
                path=request.path,
...
class UserPermissionListView(APIView):
    ...
    def get(self, request, user_id):
        ...
        if tenant is None:
            return Response({'detail': 'Tenant não disponível no contexto'}, status=status.HTTP_400_BAD_REQUEST)
        from .models import UserPermission
        qs = UserPermission.objects.filter(user=target, tenant=tenant).select_related('permission')
...
class RoleListCreateView(APIView):
    ...
    @extend_schema(request=RoleSerializer, responses={200: RoleSerializer(many=True)}, tags=['rbac'])
    def get(self, request):
        qs = Role.objects.all().prefetch_related('permissions')
        return Response([RoleSerializer(r).data for r in qs])

    @extend_schema(request=RoleSerializer, responses={201: RoleSerializer}, tags=['rbac'])
...
class RoleDetailView(APIView):
    ...
    def get(self, request, role_id):
        try:
            ...
            return Response({'detail': 'Role não encontrada'}, status=status.HTTP_404_NOT_FOUND)
        return Response(RoleSerializer(r).data)

    @extend_schema(request=RoleSerializer, responses={200: RoleSerializer}, tags=['rbac'])
...
class PermissionListCreateView(APIView):
    ...
    @extend_schema(request=PermissionSerializer, responses={200: PermissionSerializer(many=True)}, tags=['rbac'])
    def get(self, request):
        qs = Permission.objects.all()
        return Response([PermissionSerializer(p).data for p in qs])

    @extend_schema(request=PermissionSerializer, responses={201: PermissionSerializer}, tags=['rbac'])
    def post(self, request):
        ser = PermissionSerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        perm, _ = Permission.objects.get_or_create(code=ser.validated_data['code'], defaults={'description': ser.validated_data.get('description', '')})
...
class PermissionDetailView(APIView):
    ...
    @extend_schema(request=PermissionSerializer, responses={200: PermissionSerializer}, tags=['rbac'])
...
class BulkRbacApplyView(APIView):
    ...
    @extend_schema(request=BulkRbacOperationSerializer, responses={200: None}, tags=['rbac'])
    @swagger_auto_schema(
        operation_description="Aplica operações RBAC em lote (atribuições e revogações)",
    ...
    def post(self, request):
        ...
        def get_user(username):
            ...
            try:
                ...
                with schema_context('public'):
                    for field in candidate_fields:
                        try:
                            u = User.objects.filter(**{f"{field}__iexact": username}).first()
                            if u:
                                return u
                        except Exception:
                            continue
        ...
        for item in (payload.get('assign', {}).get('roles', []) or []):
            try:
                ...
                UserRole.objects.get_or_create(user_id=user_pk, role=role, tenant=tenant)
        ...
        for item in (payload.get('assign', {}).get('permissions', []) or []):
            try:
                ...
                UserPermission.objects.get_or_create(user_id=user_pk, permission=perm, tenant=tenant)
            except Exception as e:
        ...
        for item in (payload.get('revoke', {}).get('roles', []) or []):
            try:
                ...
                user_pk = getattr(user, 'pk', None)
                role = Role.objects.get(name=item['role'])
                UserRole.objects.filter(user_id=user_pk, role=role, tenant=tenant).delete()
            except Exception as e:
        ...
        for item in (payload.get('revoke', {}).get('permissions', []) or []):
            try:
                ...
                user_pk = getattr(user, 'pk', None)
                perm = Permission.objects.get(code=item['permission'])
                UserPermission.objects.filter(user_id=user_pk, permission=perm, tenant=tenant).delete()
            except Exception as e:
