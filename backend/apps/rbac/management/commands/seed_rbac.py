...
class Command(BaseCommand):
    help = (
        "Seed default RBAC roles and permissions (supports --tenant and --all-tenants)"
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "--tenant",
            help="Schema name of tenant to seed (defaults to public)",
            required=False,
        )
        parser.add_argument(
            "--all-tenants",
            action="store_true",
            help="Seed all tenants (requires django-tenants)",
            required=False,
        )
    ...
    def handle(self, *args, **options):
        ...
        def seed_in_schema(schema_name=None):
            ctx = (
                ...
                if schema_name
                else schema_context(get_public_schema_name())
            )
            with ctx:
                perms = [
                ...
                for p in perms:
                    ...
                    if created:
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"[{schema_name or 'default'}] Created permission: {perm.code}"
                            )
                        )
        ...
        if all_tenants:
            try:
                ...
                self.stderr.write(
                    "Unable to enumerate tenants; ensure apps.tenants is installed and migrations applied"
                )
        ...
        if tenant_arg:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"RBAC seed completed for tenant {tenant_arg}")
                )
                return
            except Exception:
                self.stderr.write(
                    f"Failed to seed tenant {tenant_arg}; falling back to default schema"
                )
