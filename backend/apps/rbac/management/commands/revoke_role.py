...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        deleted, _ = UserRole.objects.filter(user=user, role=role, tenant=tenant).delete()
        if deleted:
            return self.stdout.write(self.style.SUCCESS(f'Role "{role_name}" revogada do usuário "{username}" no tenant "{tenant_schema}"'))
