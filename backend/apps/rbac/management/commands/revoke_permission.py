...
class Command(BaseCommand):
    help = 'Revoke a permission from a user within a tenant schema'

    def add_arguments(self, parser):
        parser.add_argument('--username', required=True, help='Target username')
        parser.add_argument('--permission', required=True, help='Permission code (e.g., send_whatsapp)')
    ...
    def handle(self, *args, **options):
        ...
        deleted, _ = UserPermission.objects.filter(user=user, permission=perm, tenant=tenant).delete()
        if deleted:
            return self.stdout.write(self.style.SUCCESS(f'Permissão "{perm_code}" revogada do usuário "{username}" no tenant "{tenant_schema}"'))
