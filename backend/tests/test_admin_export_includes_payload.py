...
def test_admin_csv_export_includes_payload(client):
    ...
    b = AuditLog.objects.create(
        path="/y", method="POST", source="middleware", action="request", status_code=200
    )
    ...
    resp = model_admin.export_selected_as_csv(
        req, AuditLog.objects.filter(id__in=[a.id, b.id]).order_by("id")
    )
