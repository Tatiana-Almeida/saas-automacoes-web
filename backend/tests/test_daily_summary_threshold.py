...
def test_near_limit_flag_and_threshold_in_summary(client, create_tenant):
    t = create_tenant(schema_name='thresh', domain='thresh.localhost', name='Thresh', plan='free')
    ...
    # Reset via endpoint
    reset = client.post('/api/v1/core/throttle/daily/reset', data=json.dumps({'categories': ['send_whatsapp']}), content_type='application/json', HTTP_HOST=d.domain)
