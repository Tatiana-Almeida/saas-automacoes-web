...
def test_alert_quiet_period_suppresses_second_send(monkeypatch):
    ...
    log1 = AuditLog.objects.create(path='/x', method='POST', source='view', action='plan_change', tenant_schema='acme', status_code=200)
    log2 = AuditLog.objects.create(path='/y', method='POST', source='view', action='plan_change', tenant_schema='acme', status_code=200)

    cache.delete(f"audit_alert:quiet:{log1.tenant_schema}:{log1.action}")

    with override_settings(ALERT_WEBHOOK_ENABLED=True, ALERT_WEBHOOK_URL='http://example.com', ALERT_WEBHOOK_QUIET_MINUTES=60):
...
def test_alert_quiet_period_different_action_not_suppressed(monkeypatch):
    ...
    log1 = AuditLog.objects.create(path='/x', method='POST', source='view', action='rbac_change', tenant_schema='acme', status_code=200)
    log2 = AuditLog.objects.create(path='/y', method='POST', source='view', action='plan_change', tenant_schema='acme', status_code=200)
