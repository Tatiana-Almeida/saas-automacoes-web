...
def test_tenant_plan_change_updates_fields_and_logs(monkeypatch, client, create_tenant):
    ...
    with override_settings(ALERT_WEBHOOK_ENABLED=True, AUDIT_CRITICAL_ACTIONS=['plan_change']):
        resp = client.post(
        ...
        assert AuditLog.objects.filter(action='plan_change', tenant_schema='acme').exists()
        assert calls['n'] == 1
