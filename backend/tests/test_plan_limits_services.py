...
def test_daily_limit_enforcement_email(client, gen_password, create_tenant):
    t = create_tenant(schema_name='mailco', domain='mailco.localhost', name='MailCo', plan='free')
    ...
    login = client.post('/api/v1/auth/token', data=json.dumps({'username': 'mailer', 'password': pw}), content_type='application/json')
    ...
    resp2 = client.post(
        '/api/v1/email/messages/send',
        data=json.dumps({'to': 'user@example.com', 'subject': 'Hi2', 'body': 'Hello again'}),
        content_type='application/json',
        HTTP_HOST=d.domain,
    )
...
def test_daily_limit_enforcement_sms(client, gen_password, create_tenant):
    t = create_tenant(schema_name='smsco', domain='smsco.localhost', name='SmsCo', plan='free')
    ...
    login = client.post('/api/v1/auth/token', data=json.dumps({'username': 'smsuser', 'password': pw}), content_type='application/json')
    assert login.status_code == 200
    client.cookies['access_token'] = login.cookies['access_token'].value
...
def test_daily_limit_enforcement_chatbots(client, gen_password, create_tenant):
    t = create_tenant(schema_name='botco', domain='botco.localhost', name='BotCo', plan='free')
    ...
    login = client.post('/api/v1/auth/token', data=json.dumps({'username': 'botuser', 'password': pw}), content_type='application/json')
    assert login.status_code == 200
    client.cookies['access_token'] = login.cookies['access_token'].value
    ...
    resp2 = client.post(
        '/api/v1/chatbots/messages/send',
        data=json.dumps({'bot_id': 'bot-1', 'message': 'Hello again', 'session_id': 's1'}),
        content_type='application/json',
        HTTP_HOST=d.domain,
    )
    assert resp2.status_code == 429
...
def test_daily_limit_enforcement_workflows(client, gen_password, create_tenant):
    t = create_tenant(schema_name='flowco', domain='flowco.localhost', name='FlowCo', plan='free')
    ...
    login = client.post('/api/v1/auth/token', data=json.dumps({'username': 'wfuser', 'password': pw}), content_type='application/json')
    assert login.status_code == 200
    client.cookies['access_token'] = login.cookies['access_token'].value
...
def test_daily_limit_enforcement_ai(client, gen_password, create_tenant):
    t = create_tenant(schema_name='aico', domain='aico.localhost', name='AiCo', plan='free')
    ...
    login = client.post('/api/v1/auth/token', data=json.dumps({'username': 'aiuser', 'password': pw}), content_type='application/json')
    assert login.status_code == 200
    client.cookies['access_token'] = login.cookies['access_token'].value
