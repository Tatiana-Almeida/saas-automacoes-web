...
def test_send_audit_alert_disabled_returns_disabled(gen_password):
    ...
    log = AuditLog.objects.create(user=u, path='/x', method='POST', source='test', action='rbac_change', status_code=200, tenant_schema='acme')
    with override_settings(ALERT_WEBHOOK_ENABLED=False, ALERT_WEBHOOK_URL='http://localhost/hook'):
...
def test_send_audit_alert_success(monkeypatch, gen_password):
    ...
    log = AuditLog.objects.create(user=u, path='/y', method='POST', source='test', action='rbac_change', status_code=200, tenant_schema='beta')
    ...
    with override_settings(ALERT_WEBHOOK_ENABLED=True, ALERT_WEBHOOK_URL='http://localhost/hook', AUDIT_CRITICAL_ACTIONS=['rbac_change']):
        res = audit_tasks.send_audit_alert(log.id)
...
def test_post_save_signal_triggers_for_critical_action(monkeypatch):
    ...
    with override_settings(ALERT_WEBHOOK_ENABLED=True, AUDIT_CRITICAL_ACTIONS=['rbac_change']):
        ...
        AuditLog.objects.create(user=None, path='/z', method='POST', source='test', action='rbac_change', status_code=200, tenant_schema='gamma')
        assert calls['n'] == 1
