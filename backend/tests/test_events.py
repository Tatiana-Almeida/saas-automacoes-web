...
def test_plan_upgraded_event_emitted_on_plan_change(client, create_tenant):
    ...
    login = client.post('/api/v1/auth/token', data={'username': 'manager', 'password': 'Test123!'}, content_type='application/json')
    assert login.status_code == 200
    client.cookies['access_token'] = login.cookies['access_token'].value

    with override_settings(CELERY_TASK_ALWAYS_EAGER=True):
        resp = client.post(f'/api/v1/tenants/{t.id}/plan', data={'plan': 'pro'}, content_type='application/json', HTTP_HOST=d.domain)
        assert resp.status_code == 200
        assert AuditLog.objects.filter(action='event_PlanUpgraded', tenant_schema='acme').exists()
...
def test_event_retry_and_dlq_on_failure(create_tenant):
    ...
    with override_settings(CELERY_TASK_ALWAYS_EAGER=True):
        dead_letter_event.delay('FailEvent', {'tenant_schema': 'acme', 'tenant_id': 1}, reason='boom')
    assert AuditLog.objects.filter(action='event_DLQ', path__icontains='/events/DLQ/FailEvent').exists()
