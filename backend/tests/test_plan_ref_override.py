...
def test_plan_ref_daily_limits_override_settings(client, create_tenant):
    ...
    plan = Plan.objects.create(code='vip', name='VIP', daily_limits={'send_whatsapp': 1})

    # Tenant has string plan 'free' but FK plan_ref points to 'vip'
    t = create_tenant(schema_name='acme', domain='acme.localhost', name='ACME', plan='free', plan_ref=plan)
    d = Domain.objects.get(domain='acme.localhost')

    # User with permission in this tenant
    u = User.objects.create_user(username='ovr_user', password='Test123!', is_staff=True)
