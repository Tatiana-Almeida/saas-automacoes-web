...
def test_build_headers_basic_auth(monkeypatch):
    with override_settings(ELASTICSEARCH_USERNAME='u', ELASTICSEARCH_PASSWORD='p', ELASTICSEARCH_API_KEY=None):
...
def test_build_headers_api_key(monkeypatch):
    with override_settings(ELASTICSEARCH_USERNAME=None, ELASTICSEARCH_PASSWORD=None, ELASTICSEARCH_API_KEY='abc123=='):
        headers = audit_tasks._build_es_headers()
...
def test_retry_on_failure_then_success(monkeypatch):
    ...
    with override_settings(AUDIT_EXPORT_ENABLED=True, ELASTICSEARCH_URL='http://localhost:9200'):
        ...
        monkeypatch.setattr(audit_models.AuditLog.objects, 'filter', staticmethod(lambda **kw: fake_filter(**kw)))

        result = audit_tasks.export_audit_logs_to_elasticsearch(max_attempts=2, base_backoff_seconds=0.01)
