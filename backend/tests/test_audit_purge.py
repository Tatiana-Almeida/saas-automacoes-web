...
def test_purge_audit_logs_removes_old_entries(gen_password):
    ...
    old_log = AuditLog.objects.create(user=u, path='/x', method='GET', source='test', action='request', status_code=200)
    recent_log = AuditLog.objects.create(user=u, path='/y', method='GET', source='test', action='request', status_code=200)

    # Backdate the old log beyond retention window
    AuditLog.objects.filter(id=old_log.id).update(created_at=timezone.now() - timedelta(days=365))
