...
def test_tenant_plan_detail_prefers_model_daily_limits(client, create_tenant):
    ...
    t = create_tenant(schema_name='delta', domain='delta.localhost', name='Delta', plan='free')
    ...
    # Login
    login = client.post('/api/v1/auth/token', data={'username': 'viewer', 'password': 'Test123!'}, content_type='application/json')
