...
def test_retention_policy_crud_api(client, create_tenant):
    ...
    # Login
    login = client.post('/api/v1/auth/token', data={'username': 'ops', 'password': 'Test123!'}, content_type='application/json')
    assert login.status_code == 200
    client.cookies['access_token'] = login.cookies['access_token'].value

    # Create policy
    resp = client.post('/api/v1/auditing/retention-policies', data={'tenant_schema': 'acme', 'days': 45}, content_type='application/json', HTTP_HOST=d.domain)
    ...
    # Update policy
    resp = client.put(f'/api/v1/auditing/retention-policies/{pid}', data={'days': 60}, content_type='application/json', HTTP_HOST=d.domain)
    assert resp.status_code == 200
    body = resp.json()
    assert body['days'] == 60

    # Delete policy
    resp = client.delete(f'/api/v1/auditing/retention-policies/{pid}', HTTP_HOST=d.domain)
