...
def test_assign_role_requires_manage_users_permission(client, gen_password, create_tenant):
    # Setup tenant and domain (use helper to apply tenant migrations)
    t = create_tenant(schema_name='acme', domain='acme.localhost', name='ACME', plan='pro')
...
def test_assign_role_denied_without_permission(client, gen_password, create_tenant):
    t = create_tenant(schema_name='beta', domain='beta.localhost', name='Beta', plan='free')
...
def test_list_roles_scoped_by_tenant(client, gen_password, create_tenant):
    # Two tenants
    ta = create_tenant(schema_name='alpha', domain='alpha.localhost', name='Alpha', plan='pro')
    da = Domain.objects.get(domain='alpha.localhost')
    tb = create_tenant(schema_name='bravo', domain='bravo.localhost', name='Bravo', plan='pro')
