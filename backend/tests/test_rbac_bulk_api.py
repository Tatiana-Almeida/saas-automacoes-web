...
def test_bulk_rbac_apply_success(client, create_tenant):
    ...
    assert UserPermission.objects.filter(user=u1, permission__code='send_sms', tenant=t).exists()


@pytest.mark.django_db
def test_bulk_rbac_apply_partial_errors(client, create_tenant):
    t = create_tenant(schema_name='sigma', domain='sigma.localhost', name='Sigma', plan='pro')
    ...
    payload = {
        ...
        'revoke': {
            'roles': [
                {'username': 'bulk_err_u1', 'role': 'NonexistentRole'}
            ],
            'permissions': []
        }
    }
