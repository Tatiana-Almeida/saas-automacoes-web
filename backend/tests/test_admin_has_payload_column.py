...
def test_admin_has_payload_column_renders_marker(client):
    ...
    with_payload = AuditLog.objects.create(path='/', method='GET', source='events', action='event_DLQ', status_code=500, payload={'a': 1})
    without_payload = AuditLog.objects.create(path='/', method='GET', source='events', action='event_DLQ', status_code=500)
