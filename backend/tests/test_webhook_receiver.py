...
def test_webhook_valid_signature(client: Client):
    ...
    with override_settings(WEBHOOK_SECRETS={'custom': secret}):
        resp = client.post('/api/v1/core/webhooks/custom', data=body, content_type='application/json', HTTP_X_SIGNATURE=sig)
...
def test_webhook_missing_signature(client: Client):
    ...
    body = json.dumps({'event': 'ping'}).encode('utf-8')
    with override_settings(WEBHOOK_SECRETS={'custom': secret}):
        resp = client.post('/api/v1/core/webhooks/custom', data=body, content_type='application/json')
...
def test_webhook_invalid_signature(client: Client):
    ...
    body = json.dumps({'event': 'ping'}).encode('utf-8')
    with override_settings(WEBHOOK_SECRETS={'custom': secret}):
        resp = client.post('/api/v1/core/webhooks/custom', data=body, content_type='application/json', HTTP_X_SIGNATURE='bad')
        assert resp.status_code == 401
...
def test_webhook_timestamp_skew(client: Client):
    ...
    old_ts = 0
    with override_settings(WEBHOOK_SECRETS={'custom': secret}, WEBHOOK_MAX_SKEW_SECONDS=60):
        resp = client.post(
