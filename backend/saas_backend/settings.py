...
ALLOWED_HOSTS = [h.strip() for h in env('ALLOWED_HOSTS', default='127.0.0.1,localhost,.localhost,testserver').split(',')]
...
DATABASE_URL = env('DATABASE_URL', default='postgres://postgres:postgres@postgres:5432/saas')
...
TENANT_PLAN_DAILY_WARN_THRESHOLD = env.int('TENANT_PLAN_DAILY_WARN_THRESHOLD', default=80)
...
AUDIT_CRITICAL_ACTIONS = env.list('AUDIT_CRITICAL_ACTIONS', default=['rbac_change', 'plan_change', 'suspend_tenant', 'reactivate_tenant'])
ALERT_WEBHOOK_QUIET_MINUTES = env.int('ALERT_WEBHOOK_QUIET_MINUTES', default=10)
ALERT_WEBHOOK_QUIET_BYPASS_ACTIONS = env.list('ALERT_WEBHOOK_QUIET_BYPASS_ACTIONS', default=[])

# JWT cookie settings
JWT_COOKIE_NAME = env('JWT_COOKIE_NAME', default='access_token')
JWT_COOKIE_SECURE = env.bool('JWT_COOKIE_SECURE', default=False)
JWT_COOKIE_HTTPONLY = True
JWT_COOKIE_SAMESITE = env('JWT_COOKIE_SAMESITE', default='Lax')  # 'Lax' | 'None' | 'Strict'
